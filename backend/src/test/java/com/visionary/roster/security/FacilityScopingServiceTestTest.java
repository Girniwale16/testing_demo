...
class FacilityScopingServiceTest {
    ...
    // ==================== New Tests for FacilityAccessDeniedException ====================

    @Test
    void testValidateFacilityAccess_ThrowsFacilityAccessDeniedException() {
        Long userId = 1L;
        ...
        assertEquals("User does not have access to facility: " + facilityId, exception.getMessage());
        verify(userAccountRepository).findById(userId);
    }

    @Test
    void testValidateFacilityAccess_Success() {
        Long userId = 1L;
        Long facilityId = 100L;

        when(userAccountRepository.findById(userId)).thenReturn(Optional.of(testUser));

        assertDoesNotThrow(() -> facilityScopingService.validateFacilityAccess(userId, facilityId));
        verify(userAccountRepository).findById(userId);
    }

    @Test
    void testValidateFacilityAccess_StaffDeactivationScenario_CriticalFacilityScoping() {
        Long userId = 1L;
        Long staffFacilityId = 100L;
        Long unauthorizedFacilityId = 200L;

        when(userAccountRepository.findById(userId)).thenReturn(Optional.of(testUser));

        assertDoesNotThrow(() -> facilityScopingService.validateFacilityAccess(userId, staffFacilityId),
                "User should be able to deactivate staff in their own facility");

        FacilityAccessDeniedException exception = assertThrows(FacilityAccessDeniedException.class,
                () -> facilityScopingService.validateFacilityAccess(userId, unauthorizedFacilityId),
                "User should NOT be able to deactivate staff in a different facility");

        assertEquals("User does not have access to facility: " + unauthorizedFacilityId, exception.getMessage());
        verify(userAccountRepository, times(2)).findById(userId);
    }

    @Test
    void testValidateFacilityAccess_UserIdAndFacilityId_WhenUserNotFound_ShouldThrowIllegalStateException() {
        Long userId = 1L;
        Long facilityId = 100L;
    ...
    @Test
    void testValidateFacilityAccess_UserIdAndFacilityId_WhenUserHasNoFacility_ShouldThrowIllegalStateException() {
        Long userId = 1L;
        Long facilityId = 100L;
        UserAccount userWithoutFacility = new UserAccount();
        userWithoutFacility.setUserId(userId);
    ...
    @Test
    void testValidateFacilityAccess_UserIdAndFacilityId_WithMultipleFacilityChecks() {
        Long userId = 1L;
        Long validFacilityId = 100L;
        Long invalidFacilityId1 = 200L;
        Long invalidFacilityId2 = 300L;

        when(userAccountRepository.findById(userId)).thenReturn(Optional.of(testUser));

        assertDoesNotThrow(() -> facilityScopingService.validateFacilityAccess(userId, validFacilityId));

        assertThrows(FacilityAccessDeniedException.class,
                () -> facilityScopingService.validateFacilityAccess(userId, invalidFacilityId1));

        assertThrows(FacilityAccessDeniedException.class,
                () -> facilityScopingService.validateFacilityAccess(userId, invalidFacilityId2));

        verify(userAccountRepository, times(3)).findById(userId);
    }

    @Test
    void testValidateFacilityAccess_UserIdAndFacilityId_WithNullFacilityId_ShouldThrowFacilityAccessDeniedException() {
        Long userId = 1L;
    ...
    @Test
    void testValidateFacilityAccess_UserIdAndFacilityId_VerifyExceptionMessage() {
        Long userId = 1L;
        ...
        assertTrue(exception.getMessage().contains("User does not have access to facility"));
        assertTrue(exception.getMessage().contains(String.valueOf(facilityId)));
        verify(userAccountRepository).findById(userId);
    }
